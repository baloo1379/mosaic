...
"""
Create a collage of images, filling all space without cropping.
Original source: https://gist.github.com/JesseCrocker/cfd05006335c2c828a2b
Last modification: https://gist.github.com/baloo1379/d6c4e3995cc17df79ec2ffe17de259dd
Author: https://gist.github.com/JesseCrocker
...
2015-01-05 - JesseCrocker - initial release
2015-01-12 - JesseCrocker - small changes
2019-04-14 - baloo1379 - make support for Python 3
"""


def linear_partition(seq: list, k: int):
    if k <= 0:
        return []
    n = len(seq) - 1
    if k > n:
...
def linear_partition_table(seq: list, k: int):
    n = len(seq)
...
def create_collage(image_files: list, result_width: int, rows: int):
    """
    ...
    partioned_images = linear_partition(images, int(rows))
    ...
    for row in partioned_images:
        ...
        for aspect, image in row:
            ...
            scale_ratio = image_width / float(image.size[0])
            logging.debug(
                ("Image size: %f %f" % image.size) + " percent of row " + str(width_percent) + " new width " + str(
                    image_width) + " scale_ratio " + str(scale_ratio))
